...
# Files Stack Deployment with Community Alpine Docker Script
# This script deploys JDownloader2, MeTube, and Palmr to LXC 102

set -e

# Files Stack Configuration
STACK_TYPE="files"
LXC_ID=102
HOSTNAME="lxc-files-02"
...
print_info "======================================================"
print_info "Files Stack Deployment"
print_info "======================================================"
...
if [ -d "/datapool" ]; then
    # Create files stack directories
    ...
    chown -R 101000:101000 /datapool/config/{jdownloader2,metube,palmr} 2>/dev/null || true
    chown -R 101000:101000 /datapool/files 2>/dev/null || true
...
wget -q -O "$TEMP_DIR/docker-compose.yml" "$GITHUB_REPO/docker/files/docker-compose.yml"
wget -q -O "$TEMP_DIR/.env.example" "$GITHUB_REPO/docker/files/.env.example" 2>/dev/null || true
...
# Interactive setup for VNC passwords
echo ""
print_info "VNC Password Configuration"
print_info "Setting up VNC passwords for remote desktop access"
echo ""

# Generate or get existing passwords
JDOWNLOADER_VNC_PASSWORD=""
if pct exec "$LXC_ID" -- test -f "$STACK_DIR/.env" 2>/dev/null; then
    JDOWNLOADER_VNC_PASSWORD=$(pct exec "$LXC_ID" -- grep "JDOWNLOADER_VNC_PASSWORD=" "$STACK_DIR/.env" 2>/dev/null | cut -d'=' -f2 || true)
fi

if [ -z "$JDOWNLOADER_VNC_PASSWORD" ]; then
    read -s -p "Enter VNC password for JDownloader2 remote access: " JDOWNLOADER_VNC_PASSWORD
    echo ""
    if [ -z "$JDOWNLOADER_VNC_PASSWORD" ]; then
...
# Files Stack Environment Configuration
...
FILES_ROOT=/datapool/files

# VNC Configuration
JDOWNLOADER_VNC_PASSWORD=$JDOWNLOADER_VNC_PASSWORD

# Service URLs
JDOWNLOADER_URL=http://192.168.1.102:5800
METUBE_URL=http://192.168.1.102:8081
PALMR_URL=http://192.168.1.102:8090

# Setup Instructions:
# 1. JDownloader2: Access via VNC at http://192.168.1.102:5800
# 2. MeTube: Web interface at http://192.168.1.102:8081
# 3. Palmr: File manager at http://192.168.1.102:8090
# 4. Configure JDownloader2 account through VNC interface
# 5. Files are saved to /datapool/files
EOF"

print_success "Environment file created"

# Deploy services
print_info "Deploying files services..."
pct exec "$LXC_ID" -- bash -c "cd $STACK_DIR && docker compose pull"
pct exec "$LXC_ID" -- bash -c "cd $STACK_DIR && docker compose up -d"

if [ $? -eq 0 ]; then
    print_success "Files stack deployed successfully!"
    ...
    print_info "JDownloader2 VNC: http://192.168.1.102:5800"
    print_info "MeTube:           http://192.168.1.102:8081"
    print_info "Palmr:            http://192.168.1.102:8090"
    print_info "======================================================"
    print_info "Next Steps:"
    print_info "1. Access JDownloader2 via VNC to configure your account"
    print_info "2. Use MeTube to download videos from YouTube and other sites"
    print_info "3. Use Palmr to manage downloaded files"
    print_info "4. All files are stored in /datapool/files"
    print_info "======================================================"
    ...
    print_error "Failed to deploy files stack"
    exit 1
fi