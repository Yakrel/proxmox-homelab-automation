...
configure_pbs_datastore() {
    ...
    # Check if datastore already exists in PBS config
...
configure_pbs_schedules() {
    ...
    # Configure garbage collection schedule (still supported)
    if ! pct exec "$ct_id" -- proxmox-backup-manager datastore update "$datastore_name" \
        --gc-schedule "$gc_schedule"; then
        print_warning "Failed to set garbage collection schedule"
        schedule_errors=$((schedule_errors + 1))
    fi

    # Create or update prune job (modern approach instead of deprecated --prune-schedule)
    local prune_job_id="prune-daily"
    if pct exec "$ct_id" -- proxmox-backup-manager prune-job list 2>/dev/null | grep -q "^$prune_job_id"; then
        # Update existing prune job
        if ! pct exec "$ct_id" -- proxmox-backup-manager prune-job update "$prune_job_id" \
            --schedule "daily" \
            --store "$datastore_name" \
            --keep-daily "$keep_daily" \
            --keep-weekly "$keep_weekly" \
            --keep-monthly "$keep_monthly" \
            --comment "Daily prune job for backup retention"; then
            print_warning "Failed to update prune job"
            schedule_errors=$((schedule_errors + 1))
        fi
    else
        # Create new prune job
        if ! pct exec "$ct_id" -- proxmox-backup-manager prune-job create "$prune_job_id" \
            --schedule "daily" \
            --store "$datastore_name" \
            --keep-daily "$keep_daily" \
            --keep-weekly "$keep_weekly" \
            --keep-monthly "$keep_monthly" \
            --comment "Daily prune job for backup retention"; then
            print_warning "Failed to create prune job"
            schedule_errors=$((schedule_errors + 1))
        fi
    fi
...
configure_pbs() {
    ...
    keep_daily=$(yq -r ".stacks.backup.pbs_keep_daily" "$WORK_DIR/stacks.yaml")
    keep_weekly=$(yq -r ".stacks.backup.pbs_keep_weekly" "$WORK_DIR/stacks.yaml")
    keep_monthly=$(yq -r ".stacks.backup.pbs_keep_monthly" "$WORK_DIR/stacks.yaml")
