...
get_latest_template() {
    ...
    local template=$(pveam list "$STORAGE_POOL" | awk "/${template_type}/ {print \$1}" | sort -V | tail -n 1 | sed "s|^${STORAGE_POOL}:vztmpl/||")
    
    # If not found locally, download the latest
    if [ -z "$template" ]; then
        print_info "Downloading latest ${template_type} template" >&2
        local download_template=$(pveam available | awk "/${template_type}/ {print \$2}" | sort -V | tail -n 1)
        ...
        template=$(pveam list "$STORAGE_POOL" | awk "/${template_type}/ {print \$1}" | sort -V | tail -n 1 | sed "s|^${STORAGE_POOL}:vztmpl/||")
