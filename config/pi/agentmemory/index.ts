...
export default function agentmemoryExtension(pi: ExtensionAPI) {
  ...
  let currentProject = resolveProject(currentCwd);
  let lastPrompt = "";
  ...
  pi.registerCommand("agentmemory-status", {
    description: "Check the configured Agentmemory server health",
    handler: async (_args, ctx) => {
      const health = await getHealth();
      if (!health) {
        ctx.ui.notify(`agentmemory is unreachable at ${configuredBaseUrl()}`, "warning");
        return;
      }
      ctx.ui.notify(
  ...
  pi.registerTool({
    ...
    async execute() {
      ...
      if (!health) {
        return {
          content: [{ type: "text", text: `agentmemory is unreachable at ${configuredBaseUrl()}` }],
          details: { ok: false },
        };
      }
      return {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    currentProject = resolveProject(currentCwd);
    lastPrompt = "";
    sessionContext = "";
    bootstrapRecall = "";
