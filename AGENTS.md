...
### Environment Secrets
The repository uses encrypted `.env.enc` files. Use `ENV_ENC_KEY` from GitHub secrets for decryption/encryption with openssl. Always commit only `.env.enc` files, never decrypted `.env` files.
