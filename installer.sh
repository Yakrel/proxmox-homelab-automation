...
run_ansible_playbook() {
    ...
    if [[ "$playbook_command" =~ deploy\.yml ]]; then
        ...
        if [ -n "$stack_name" ]; then
            ...
            local template_name
            template_name=$(ensure_template_available "$template_type")
    ...
    # Execute the command
    pct exec "$CONTROL_CT_ID" -- bash -l -c "cd $PLAYBOOK_DIR && PROXMOX_API_TOKEN_SECRET='$CURRENT_TOKEN_SECRET' ansible-playbook $vault_param --extra-vars 'proxmox_api_token_secret=$CURRENT_TOKEN_SECRET' $playbook_command"
}
